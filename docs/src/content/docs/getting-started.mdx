...
In this guide, we'll build a test that allows us to observe how a serializable read can return stale data when a node
...
<Aside type="note" title="Why do I need sudo?">
    ...
    Currently each test needs root access, but it is planned to have a dedicated pakostii daemon that will allow keeping test code in non-root user mode.
</Aside>
...
Everything starts with a cluster specification that lets you define nodes. Each node is an abstraction over a Linux network namespace, which allows
...
func main() {
	...
	// Preparing the names of nodes for the etcd cluster. The name of the node is also used as its hostname address
	...
	for i, host := range etcdHosts {
		...
		etcd := spec.Etcd(
			...
			// configuration helps set up etcd1 in a way to discover other nodes
			spec.EtcdConfig{
				Name:         host,
...
With this block of code we've programmatically defined a cluster with 3 nodes running etcd. We've assigned each etcd instance to a specific AZ, which will
...
// Besides the cluster spec, each test needs a name that is used to identify test runs later.
...
started and we can see our "Hello, World!" message logged as well. After that, our test returns `nil`, which means the test passed.
...
During runtime, pakostii collects stdin and stdout for each container and stores the logs for each service under the node directory the service belongs to.
...
We want to interact with the etcd instances using the Go etcd client library, but since all nodes in a cluster run in an
isolated LAN that is not accessible from the host network by default, we will need to set up port forwarding for our etcd instances.
...
This piece of code will ask pakostii to set up port forwarding by assigning a random local port that will forward traffic to `etcd1:2379` and `etcd2:2379`.
Using the port forwarding, we set up 2 etcd clients, one for each node.
...
// We'll wait 5 seconds to account for leader re-election in the majority
...
This happens because etcd client reads are linearizable, which means the node must sync its state with the cluster before responding to the client.
...
That's it! We successfully set up an etcd cluster, isolated one node and got a stale read from it. Of course, we haven't found any bugs today,
but we've learned about how etcd handles network issues and cluster split-brain scenarios. The example code has been simplified for clarity purposes,
